 ...
 * <p>
 * 对于转载因子的默认0.75，目的是在时间和空间上做一个平衡。大的转载因子，会提高空间利用率，但是因为数据太多会导致查询时间增加。
 * 因此，在设置loadFactory和初始容量的时候，需要考虑效率问题和rehash问题，合理的loadFactory控制好了threshHold（这个值主要用于控制扩容），
 * 可以减少rehash操作。
 *
 *
 *
 ...
 * <p>
 * 如果能预见性的知道hash的数量，尽量设置较大的容量，避免给rehash。
 *
 ...
 * <p>
 * synchronizedMap 如果把传统的HashMao包装成同步的
 *
...
public class HashMap<K, V> extends AbstractMap<K, V>
    ...
    /**
     * The default initial capacity - MUST be a power of two.  // 这里系统会保证是2的冥
     */
     ...
     * <p>
     * 一个hashMap的put操作，会做几件事情呢？
     * 1、resize 操作 （如果为空，那么就会分配资源；如果达到Threshold的阈值了，那么就会rehash）
     * 2、放置key-value，hashMap抽象了一个Node（hash，key，value） 不同的key可能在同一个hash槽，所以这叫碰撞。
     *   2.1 如果某一个hash槽的拉链大于8，那么要还是用红黑树改写链表。 （当前，如果在树化之后，又重新小于6时，会继续把树化去掉）。
     *       Q：引入树节点后，会带来什么影响？ 树有父指针，左右指针，pre指针的开销，因此空间比较多，原来的树节点，只有一个next指针，空间上比树节点小。
     *
     * @param hash         hash for key
     * @param key          the key
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            // notice resize需要 rehash，类似于数据copy这么一回事，这里对于大数据量，可能比较的耗时了。
            n = (tab = resize()).length;
    ...
    final Node<K, V>[] resize() {
        ...
        } else if (oldThr > 0) // initial capacity was placed in threshold 仅仅在指定了capacity初始化的时候
        ...
        threshold = newThr;  // 这里会更新threshold的值
        @SuppressWarnings({"rawtypes", "unchecked"})
        Node<K, V>[] newTab = (Node<K, V>[]) new Node[newCap]; //
        table = newTab;
        if (oldTab != null) {  // 下面的工作就是rehash。 从这个rehash可以看出，
