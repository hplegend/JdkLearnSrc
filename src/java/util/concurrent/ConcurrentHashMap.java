    ...
    static final class TreeBin<K, V> extends Node<K, V> {
        ...
        final TreeNode<K, V> putTreeVal(int h, K k, V v) {
            ...
            for (TreeNode<K, V> p = root; ; ) {  // 循环开始遍历。p=root根节点。
                int dir, ph;
                K pk;  // pk之前键的指针， dir 表示插入的方向左或者右，ph节点的hash值
         ...
         * call.  This is messier（混乱的） than typical red-black deletion code
         * because we cannot swap the contents of an interior node
         * with a leaf successor that is pinned（连接） by "next" pointers
         * that are accessible independently of lock. So instead we
         * swap the tree linkages （不是交换节点，而是交互连接？？？）.
         *并没有删除中间节点，也没有交换，而是交换了tree的链接
         * @return true if now too small, so should be untreeified
         */
        final boolean removeTreeNode(TreeNode<K, V> p) {
            TreeNode<K, V> next = (TreeNode<K, V>) p.next; // 这里牛鼻了，同时保留了next和pre，left及right指针。
            TreeNode<K, V> pred = p.prev;  // unlink traversal pointers // next 和 pred
            TreeNode<K, V> r, rl;  // r--> root节点， rl-->root的左孩子
            if (pred == null)  // 删除的是根节点？或者说第一个节点？
                first = next;
            else
                pred.next = next;  // 这里正好是在双向列表上断开（删除），也就是原来的结构
            if (next != null)
                next.prev = pred;  // end of 断开连接
            if (first == null) {
                root = null;
                return true;
            } // 感觉上面的代码就是删除节点
            ...
            try {
                ...
                if (pl != null && pr != null) {
                    TreeNode<K, V> s = pr, sl;  // s，后继节点； sl 后继的左孩子
                    while ((sl = s.left) != null) // find successor
                        s = sl;
                    boolean c = s.red;
                    s.red = p.red;
                    p.red = c; // swap colors  why? 交换颜色，那颜色岂不是变化了？
                    TreeNode<K, V> sr = s.right;  // s后继节点
                    TreeNode<K, V> pp = p.parent;  // p单前待删除节点
                    if (s == pr) { // p was s's direct parent   s节点是p节点的直接后继
                        p.parent = s;
                        s.right = p; // p作为s的右孩子？ p节点小于等于s，怎么放到右边？？ pulze。 从下面的代码看，放左放右还有依据当前的位置
                    } else {
                        ...
                        if ((p.parent = sp) != null) {
                            if (s == sp.left)
                                ...
                                sp.right = p;
                        } // 这里是p（单前待删除接）直接append到了s（后继节点）？？
                        if ((s.right = pr) != null)
                            pr.parent = s;  // s的值小于pr，因此才能做父节点。这里要特别注意大小顺序问题！！！！
                    }
                    p.left = null;
                    if ((p.right = sr) != null)
                        sr.parent = p;
                    ...
                    replacement = p;   // 前面的所有这一坨代码，视乎都是在交换链接（tree的链接），而非交换node值。正如注释所说的那样
                if (replacement != p) {
